...
CONFIG = {
    ...
    "etf_weight": 15.0,
    "signal2_manual": {"status": "watch", "note": "首個催化已出且不及格：海力士Q2淨利+1242%、營益+557%仍低於市場高預期，資本開支上調50%至310億美元反引發存儲供給過剩疑慮（7/28-29暴跌導火線）——正面催化未落地；下一站看美系雲巨頭財報與資本開支指引、三星財報"},
    "signal3_manual": {"status": "yellow", "note": "7/16已落地：暫停單股槓桿ETF新發、開戶門檻1,000萬→3,000萬韓元、最小交易單位1→20股；7/31門檻正式生效（不符門檻者的最後一波平倉壓力測試）、8/5起僅認現金。僅限新增、未觸16檔存量與每日再平衡機制——治標；監管另研議市場穩定基金／養老金等長期資金入市（若落地屬正面）。後續看限再平衡、壓槓桿倍數、提資本要求"},
