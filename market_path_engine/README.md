...
The `rebound-stats` and `crash-rebound-screen` pages remain visible in the master inventory, but their catalog `bundle` flag is intentionally disabled, so they are not collected or sent into the ChatGPT analysis context.
...
The shared page password is held in memory only while the bundle is being built and is explicitly excluded from the export. The dashboard does not automatically transmit data to ChatGPT. The recommended workflow is: download the complete JSON, copy the generated Traditional Chinese analysis instruction, open ChatGPT, click the `+` / paperclip attachment control at the lower-left of the conversation box, select the JSON from the Downloads folder, wait for the attachment to finish uploading, paste the copied instruction, and send. A copy-all option is also provided for smaller bundles, but JSON upload is the recommended path for large histories.
